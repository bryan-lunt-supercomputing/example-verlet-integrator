...
int main(int argc, char **argv){
	...
	lattice_t ghosts_grid = create_lattice(0.36,
											(lattice_position_t){.x=2.0,.y=-3.0},
											(lattice_position_t){.x=2.5,.y=3.0}
										);

	int N_particles = fill_this.N_total;
	int N_ghosts = ghosts_grid.N_total;
	...
	for(j = 0;j<N_ghosts;j++){
		phys_particle_t p;
		lattice_position_t foobar = get_lattice_position(&ghosts_grid,j);
		p.p = (phys_vector_t){ .x = foobar.x, .y = foobar.y };
		p.v = (phys_vector_t){ .x = 0.0, .y = 0.0 };
		all_ghosts[j] = p;
	}

	fprintf(stderr, "GHOST_GRID %d %d |%d|\n",ghosts_grid.N_x, ghosts_grid.N_y, ghosts_grid.N_total);

	maxwell_boltzmann(&my_sim, 1.0*KB_AGPERKELVIN_NM2PERS2, all_particles, N_particles );


	printf("%d\n",N_particles+N_ghosts);//Number of particles
	printf("\n");//Comment line
	for(int j = 0;j<N_particles;j++)
		printf("Ar %f %f\n",j, all_particles[j].p.x, all_particles[j].p.y);
	for(int j = 0;j<N_ghosts;j++)
		printf("Ar %f %f\n",j, all_ghosts[j].p.x, all_ghosts[j].p.y);
