...
int main(int argc, char **argv){
	...
	lattice_t fill_this = create_lattice(0.47,
											{.x=-1.0,.y=-1.0,.z=-1.0},
											{.x=2.0,.y=2.0,.z=2.0}
										);
	...
	for(j = 0;j<N_particles;j++){
		...
		p.p = (phys_vector_t){ .x = foobar.x, .y = foobar.y, .z = foobar.z };
		p.v = (phys_vector_t){ .x = 0.0, .y = 0.0, .z = 0.0 };
		all_particles[j] = p;
	}

	maxwell_boltzmann(&my_sim, 50.0*KB_AGPERKELVIN_NM2PERS2, all_particles, N_particles );
	...
	for(int j = 0;j<N_particles;j++)
		fprintf(stderr, "Ar %f %f %f \n",j, all_particles[j].v.x, all_particles[j].v.y, all_particles[j].v.z);
	}
	...
	printf("\n");//Comment line
	for(int j = 0;j<N_particles;j++)
		printf("Ar %f %f %f \n",j, all_particles[j].p.x, all_particles[j].p.y, all_particles[j].p.z);
